...
t1 = Tip.create!({user_id: julia.id, city_id: berlin.id, category_id: misc.id, content: "Buy a bottle of Rotkaeppchen prosecco (a vestige of the GDR) and sit on the canal"})
t2 = Tip.create!({user_id: georgina.id, city_id: sf.id, category_id: food.id, content: "Philz original on 24th and folsome"})
t3 = Tip.create!({user_id: georgina.id, city_id: sf.id, category_id: misc.id, content: "Rare device in nopa. It's a fun shop"})
t4 = Tip.create!({user_id: julia.id, city_id: berlin.id, category_id: culture.id, content: "There's an abandoned amusement park in Neukoelln called Spreepark. You can easily slip through the fence to explore"})

Favorite.create!({user_id: julia.id, tip_id: t2.id})
Favorite.create!({user_id: georgina.id, tip_id: t1.id})
Favorite.create!({user_id: julia.id, tip_id: t3.id})
